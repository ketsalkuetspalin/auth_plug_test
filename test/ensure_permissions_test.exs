defmodule Resuelve.Plug.EnsurePermissionTest do
  ...
  test "doesnt call unauthorized when permissions are present", %{conn: conn} do
    pems = Resuelve.Permissions.to_value([:read, :write])
    claims = %{"pem" => %{"default" => pems}}

    expected_conn =
      conn
      |> Resuelve.Plug.set_claims({:ok, claims})
  ...
  test "is invalid when missing a requested permission", %{conn: conn} do
    pems = Resuelve.Permissions.to_value([:read])
    claims = %{"pem" => %{"default" => pems}}
  ...
  test "is invalid when claims don't include the pem key", %{conn: conn} do
    pems = Resuelve.Permissions.to_value([:other_read], :other)
    claims = %{"pem" => %{"default" => pems}}
  ...
  test "is invalid when all permissions are not present", %{conn: conn} do
    pems = Resuelve.Permissions.to_value(
      [:read, :write, :update, :delete],
      :default
    )
    other_pems = Resuelve.Permissions.to_value([:other_write], :other)
  ...
  test "is valid when all permissions are present", %{conn: conn} do
    ...
    other_pems = Resuelve.Permissions.to_value(
